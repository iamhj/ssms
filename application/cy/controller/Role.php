...
class Role extends Base
{
    ...
    public function roleTree()
    {
        ...
        $menu=db("cy_role")->where("role_id",$role_id)->value("menus");
        //去掉加载icon字段是避免ztree中图标不显示
        $list = db('cy_menu')->field('icon', true)->select();
    ...
    public function roleForm()
    {
        if (request()->isPost()) {
            ...
            if ($data['role_id'] == null) {
                db('cy_role')->insert($data);
                return $this->success('添加成功！');
            } else {
                db('cy_role')->update($data);
    ...
    public function roleDel()
    {
        ...
        $subuser = db('cy_user')->where("role_id", $id)->find();
        if ($subuser) {
            return $this->error('该角色分配有用户，删除失败!');
        } else {
            db('cy_role')->delete($id);
