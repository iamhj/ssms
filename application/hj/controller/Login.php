...
//                if ($user['status'] == 1 && $user['password'] == md5($data['password'])) {
                if ($user['password'] == md5($data['password'])) {
                    ...
                    //db('hj_user')->where('user_id', $user['user_id'])->update(['login_time' => time()]);
                    //db('hj_user')->where('user_id',$user['user_id'])->setField('online',1);
                    return $this->success('登录成功!', url('hj/index/index')); //信息正确
    ...
    public function register()  //对应view\login\register.html
    {
        if (request()->isPost()) {
            ...
            if ($user) {
                ...
                $phone = db('hj_user')->where('phone', $data['phone'])->find();
                if ($phone) {
...
//                    $data['role_id'] = 2;   //默认注册为“买家”
    ...
    public function resetPwd()
    {
        if (request()->isPost()) {
//            $data1 = input('post.');
//            $phone = db('hj_user')->where('phone', $_POST['phone'])->find();
//            $phone = $_POST['phone'];
//            $data = input('post.');
//            $data['password'] = md5($data['password']);
            $data = [
