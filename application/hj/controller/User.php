...
class User extends Base
{
    public function index()
    {
        if (request()->isAjax()) {
            ...
            $list = db('hj_user')->alias('u')
                ->join('hj_role r', 'u.role_id = r.role_id', 'left')
                ->field('u.*,r.name')//查询结果
                ->where($where)
    ...
    public function userForm()
    {
        if (request()->isPost()) {
            ...
            if ($data['user_id'] == null) {
                $user = db('hj_user')->where(['user_name' => $data['user_name']])->find();
                if ($user) {
                    $this->error('用户名已经存在！');
                }
                $data['password'] = md5($data['password']);
                $data['online']=0;
                db('hj_user')->insert($data);
    ...
    public function userInfo()
    {
        if (request()->isPost()) {
            $data = input('post.');
            db('hj_user')->strict(false)->update($data);
            return $this->success('用户信息更新成功！');

        } else {
            $user_id=session('user_id');
            $data = db('hj_user')->where('user_id=' . $user_id)->find();
    ...
    public function userDel()
    {
        ...
        db('hj_user')->delete(input('post.user_id/a'));
    ...
    public function userStatus()
    {
        ...
        if (db('hj_user')->where('user_id', input('post.user_id'))->update(['status' => $status]) !== false) {
    ...
    {
        db('hj_user')->where('user_id', input('post.user_id'))->update(['password' => md5("1")]);
        return $this->success('重置密码成功，新密码为1!');
    }

    public function modiPwd(){
        $data=input("post.");
        $user = db('hj_user')->where('user_id', session("user_id"))->find();
        if($user['password'] != md5($data['oldPsw'])){
            ...
            /*unset($data['oldPsw']);
            unset($data['newPsw']);
            if (db('hj_user')->update($data)!==false) {*/

            //strict  关闭严格检查字段名  https://www.kancloud.cn/manual/thinkphp5/162902
            if (db('hj_user')->strict(false)->update($data)!==false){  //
