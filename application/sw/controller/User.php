...
class User extends Base
{
    public function index()
    {
        if (request()->isAjax()) {
            ...
            $where = 'user_id!=1';
            if ($key) {
                $where .= " and (user_name like '%" . $key . "%'";
                $where .= " or real_name like '%" . $key . "%')";
            }
            if ($role_id > 0) {
                $where .= " and (r.role_id=" . $role_id . ")";
            }

            $list = db('sw_user')->alias('u')
                ->join('sw_role r', 'u.role_id = r.role_id', 'left')
                ->field('u.*,r.name')//查询结果
                ->where($where)
                // ->select('role_id,u.user_id')
                ->paginate($limit)
                ->toArray();
    ...
    public function userForm()
    {
        if (request()->isPost()) {
            ...
            if ($data['user_id'] == null) {
                $user = db('sw_user')->where(['username' => $data['username']])->find();
                if ($user) {
                    $this->error('用户名已经存在！');
                }
    ...
    public function userInfo()
    {
        if (request()->isPost()) {
            $data = input('post.');
            db('sw_user')->strict(false)->update($data);
            return $this->success('用户信息更新成功！');

        } else {
            $user_id=session('user_id');
            $data = db('sw_user')->where('user_id=' . $user_id)->find();
    ...
    public function userDel()
    {
        ...
        db('sw_user')->delete(input('post.user_id/a'));
    ...
    public function userStatus()
    {
        ...
        if (db('sw_user')->where('user_id', input('post.user_id'))->update(['status' => $status]) !== false) {
    ...
    {
        db('sw_user')->where('user_id', input('post.user_id'))->update(['password' => md5("123456")]);
        return $this->success('重置密码成功，新密码为123456!');
    }

    public function modiPwd(){
        $data=input("post.");
        $user = db('sw_user')->where('user_id', session("user_id"))->find();
        if($user['password'] != md5($data['oldPsw'])){
            ...
            /*unset($data['oldPsw']);
            unset($data['newPsw']);
            if (db('sw_user')->update($data)!==false) {*/

            //strict  关闭严格检查字段名  https://www.kancloud.cn/manual/thinkphp5/162902
            if (db('sw_user')->strict(false)->update($data)!==false){  //
